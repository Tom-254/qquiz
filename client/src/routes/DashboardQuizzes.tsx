...
import { useGetUserQuizGroupsQuery } from "../api/api";
import {
  DeleteIcon,
  EditIcon,
  PrivateIcon,
...
const DashboardQuizzes = () => {
  ...
  } = useGetUserQuizGroupsQuery(page);
  ...
  console.log(userQuizData.next_num);
    ...
    <section className="flex flex-col gap-[16px] px-[10px] sm:px-[24px] pb-[70px] pt-[16px] overflow-y-auto max-h-[80vh]">
      ...
      <div className="flex flex-col gap-[16px] bg-white rounded-[24px] w-full p-[24px]">
        ...
        <div className="flex flex-col gap-[15px] sm:flex-row sm:items-center justify-between mt-[50px]">
          <Button
            disabled={userQuizData.prev_num === null}
            size="icon"
            type="tertiary"
            onClick={() => setPage(userQuizData.prev_num)}
          >
            Previous
          </Button>
          ...
          <Button
            disabled={userQuizData.next_num === null}
            size="icon"
            type="tertiary"
            onClick={() => setPage(userQuizData.next_num)}
          >
            Next
          </Button>
        </div>
